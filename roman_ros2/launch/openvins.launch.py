...
launch_args = [
    ...
    DeclareLaunchArgument(name='robot', description='name of the robot used for topics')
]
...
def launch_setup(context):
    ...
    topic_remappings = [
        (['/imu0'], ['/hercules_node/', robot, '/imu/imu']),
        (['/cam0/image_raw'], ['/hercules_node/', robot, '/front_center_Scene/image']),
    ]
    node1 = Node(
        package="ov_msckf",
    ...
    node2 = Node(
        ...
        arguments=[
            "-d"
            + os.path.join(
                get_package_share_directory("roman_ros2"), "rviz", "openvins_rviz.rviz"
            ),
            "--ros-args",
            "--log-level",
            "warn",
